...
format: `type glu_op k n m n_mats n_used`, one line, matching test_
...
// numeric fields (type glu_op k n m n_mats n_used), same convention as
// make_test_cases_from_file()'s own numeric-field format. This is the
// runtime-parameterization hook HI119 needs: a Python evidence producer
// can derive these seven values from a real BigCherry dispatch signature
// (ne0/ned/n_expert/n_expert_used/src0_type/glu_op) without recompiling
// test-backend-ops for every new shape. See
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_from_moe_glu_file(const char * path) {
    ...
    if (std::getline(f, line)) {
        ...
        int64_t k, n, m;
        int n_mats, n_used;
        iss >> k >> n >> m >> n_mats >> n_used;

        if (!iss.fail()) {
            test_cases.emplace_back(new test_bigcherry_moe_glu_fusion(type, glu_op, k, n, m, n_mats, n_used));
        }
    }
...
_USAGE_NEW = (
    ...
    '    printf("    --moe-glu-file <path> reads one line (type glu_op k n m n_mats n_used) and runs\\n");\n'
