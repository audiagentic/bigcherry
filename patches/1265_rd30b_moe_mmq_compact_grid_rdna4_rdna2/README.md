...
Model per card: gfx1201 (32 GB) uses the contract's Qwen3.6-35B-A3B MTP model; gfx1030 (RX 6900 XT, 16 GB) cannot hold it (~17 GB) and runs the speed lanes on gpt-oss-20B (MoE, 32 experts, top-4), a weaker test of the 256-expert compaction. The bit-identity check uses synthetic 256-expert shapes on both cards, so it is unaffected.
