"""RD30B (1265): patch-local validation producer, adapted from 1237's.

RD30B-MOE-MMQ-COMPACT-GRID-RDNA4-RDNA2 is a performance contract with a
bit_identical correctness obligation on gfx1201 and gfx1030. Control is the
validated baseline, which already carries 1237 (gated off on these
architectures); the subject adds 1265, which opens the gate.
...
- correctness (``bit_identical``): test-backend-ops on the exact 256-expert
  ...
  test-backend-ops evidence patches; they differ only by 1265.
...
- performance: standard-scaffold llama-bench pair (control = validated BC,
  subject = + 1265), 10 paired rounds each: pp512 on the MoE model
...
_CONTRACT_ID = "RD30B-MOE-MMQ-COMPACT-GRID-RDNA4-RDNA2"
_ARCHITECTURES = ("gfx1201", "gfx1030")
_SUBJECT_PATCH = "1265_rd30b_moe_mmq_compact_grid_rdna4_rdna2"
_EVIDENCE_PATCHES = (
...
_SUBJECT_TRACE_ARTIFACT = "rd30b-subject-trace.log"
_CONTROL_TRACE_ARTIFACT = "rd30b-control-trace.log"


def _fail(message: str) -> vp.ValidationProducerError:
    return vp.ValidationProducerError(f"rd30b: {message}")
...
def run(ctx: vp.ProducerContext) -> vp.ProducerResult:
    ...
    _ARCHITECTURE = support.single_architecture(ctx, _ARCHITECTURES, label="rd30b")
    ...
    if len(devices) != 1:
        raise _fail(f"--device-map must select exactly one {_ARCHITECTURE} device; got {len(devices)}")
    device = devices[0]
    ...
    scratch_dir = ctx.workdir / "scratch" / "rd30b-correctness"
    ...
    trigger_evidence = experiment_execution.trigger_evidence_from_marker_probe(
        lane_id="rd30b-prefill-subject", role="positive", positive_hit=trigger_hit
    )

    return vp.ProducerResult(
        correctness={
            "disposition": "passed" if bit_identical_result.passed else "failed",
            "mechanism": "rd30b-mul-mat-id-bit-identical",
