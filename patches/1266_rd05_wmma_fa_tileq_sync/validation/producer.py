"""Validation producer for 1266 / PRBE110 RD05."""
...
_LABEL = "prbe110-rd05"
_CONTRACT_ID = "PRBE110-RD05-WMMA-TILEQ-SYNC"
_ARCHITECTURES = ("gfx1201",)
_MODEL_REF = "tierA-qwen4b-q6k"
_MARKER_REGEX = r"BIGCHERRY_PATCH_HIT patch=1266_rd05_wmma_fa_tileq_sync path=wmma_f16_dispatch contract=PRBE110-RD05-WMMA-TILEQ-SYNC"
_ROUNDS = support.contract_paired_rounds(_CONTRACT_ID)
_MEASUREMENT = support.contract_measurement(_CONTRACT_ID)
_PROMPT = " ".join(["Flash attention computes causal attention over the current context."] * 8)
_N_PREDICT = 64
...
def run(ctx: vp.ProducerContext) -> vp.ProducerResult:
    ...
    control_log = logs / "prbe110-rd05-control-server.log"
    subject_log = logs / "prbe110-rd05-subject-server.log"
    ...
    try:
        comparison = full_vocab.compare_servers(
            ...
            scratch_dir=ctx.workdir / "scratch" / "prbe110-rd05",
        )
    ...
    subject_trace = ctx.runtime.write_text_artifact(name="prbe110-rd05-subject.log", text=support.compact_log(subject_text))
    control_trace = ctx.runtime.write_text_artifact(name="prbe110-rd05-control.log", text=support.compact_log(control_text))
    ...
    outcome = ctx.runtime.run_paired_llama_benchmark(
        ...
        pairs=_ROUNDS,
        log_context="prbe110-rd05-lanes",
        device=device,
        combined=True,
    )
    positive_effect, positive_run = support.lane_effect(
        outcome, workload="decode", metric="tg128", role="positive", rounds=_ROUNDS, label=_LABEL
    )
    control_effect, control_run = support.lane_effect(
        outcome, workload="prefill", metric="pp512", role="control", rounds=_ROUNDS, label=_LABEL
    )
    performance = ctx.runtime.write_artifact(
        name="prbe110-rd05-performance.json",
        payload={
            "schema_version": 1,
            ...
            "positive": {"metric": "tg128", "effect": dataclasses.asdict(positive_effect), "runs": list(positive_run.runs), "stats": dict(positive_run.stats)},
            "control": {"metric": "pp512", "effect": dataclasses.asdict(control_effect), "runs": list(control_run.runs), "stats": dict(control_run.stats)},
            "trigger": {"subject_hit": subject_hit, "control_hit": control_hit},
        },
    )
    ctx.runtime.write_artifact(
        name="prbe110-rd05-correctness.json",
    ...
    trigger = experiment_execution.trigger_evidence_from_marker_probe(
        lane_id="prbe110-rd05-subject", role="positive", positive_hit=trigger_hit
    )
    ...
    return vp.ProducerResult(
        ...
        emitted_artifacts=frozenset({"prbe110-rd05-correctness.json", "prbe110-rd05-performance.json", "prbe110-rd05-subject.log", "prbe110-rd05-control.log"}),
    )
