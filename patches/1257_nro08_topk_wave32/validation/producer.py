"""PNRO07 (NRO08/1257): patch-local validation producer.

NRO08-TOPK-WAVE32 (runs with --common-patches 1256_nro07_topk_hybrid). The fork's small-row route is compiled only for
HIP >= 7.15, so runs use the ROCm 10.x toolchain (HIP 7.16); both arms are
built with it. Control = baseline, subject = + 1257 (standard
scaffold).
...
- activation: the subject test-backend-ops output carries at least one
  1257 route marker (small / parallel_radix); the control none.
...
_LABEL = "nro08"
_PATCH_TAG = "1257_nro08"
_ARCHITECTURES = ("gfx1100", "gfx1201")
_CONTRACT_ID = "NRO08-TOPK-WAVE32"
...
def run(ctx: vp.ProducerContext) -> vp.ProducerResult:
    ...
    pair = ctx.runtime.build_pair(
        ...
        common_extra_patches=("1256_nro07_topk_hybrid",),
