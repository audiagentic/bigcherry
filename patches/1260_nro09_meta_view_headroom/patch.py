...
from bigcherry.patch.patchset import Patch, Edit


def build() -> Patch:
    return Patch(
        id="1260_nro09_meta_view_headroom",
        title="Increase Meta compute-container view headroom for recurrent/MTP graphs",
        description=(
            "Increase compute_headroom from 16 to 80 in ggml-backend-meta.cpp "
            "to cover recurrent+MTP graphs with n_rs_seq up to 8 and up to 4 "
            "recurrent layers (2*(8+1)*4 = 72 views). The change is "
            "correctness-scoped: it only increases the compute buffer size."
        ),
        requires=("1001_hip_internal_allreduce",),
        edits=(
            Edit(
