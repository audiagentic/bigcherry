...
patch edits that carry them, then proves the emitted C++ is behaviourally
correct: the 16-case `mul_mat_q_launch_forced_J` switch routes to exactly the
...
otherwise through one shared launcher; and the HI71 dense-shape-aware tail
envelope rejects a forced J outside upstream's own J_max search over the real
batch width and asserts on a missing real `ncols_max`. The report identifies
these as host fixture checks, not completed HIP binary or GPU architecture
evidence.
