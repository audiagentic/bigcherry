...
_CHECK_ID = "campaign-source-selection"
...
def _load_patch(package_root: Path) -> Any:
    ...
    spec = importlib.util.spec_from_file_location("bigcherry_0110_validation_patch", patch_path)
...
def _source_list(module: Any, root: Path) -> tuple[str, bool, bool]:
    ...
    insert) and return (list, first, second). 0110's HIP_BACKEND_PATCH has
    exactly one edit (unlike 0100's split options+backend patches before
    PA27, this package was never composed with a second, unrelated edit in
    the same FilePatch)."""
    ...
    if not first.ok or not first.changed:
        raise RuntimeError(f"campaign source patch did not apply: {first.results!r}")
    second = apply_patch(patch, root)
    if not second.ok or second.changed:
        raise RuntimeError(f"campaign source patch is not idempotent: {second.results!r}")
    text = source.read_text(encoding="utf-8")
    start = text.index("    set(_BC_CAMPAIGN_SOURCES")
    end = text.index("    list(APPEND GGML_SOURCES_ROCM ${_BC_CAMPAIGN_SOURCES})", start)
...
def check(ctx: Any) -> ValidationResult:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="campaign-selection-", dir=run_dir) as temp:
            work = Path(temp)
...
"""
            script_path = work / "campaign-selection.cmake"
            ...
            for name, enabled, expected in cases:
                # GGML_HIP_DISPATCH_REPLAY=ON simulates 0100_cmake_options
                # being applied alongside this package (0110's own outer
                # guard reads GGML_HIP_AUTOTUNE OR GGML_HIP_DISPATCH_REPLAY,
                # a foreign 0100-owned variable) -- this check is scoped to
                # 0110's OWN source-selection logic, not the cross-package
                # plumbing sentinel checks (covered by patch-mechanics tests).
            ...
            report_path = work / "campaign-selection.json"
            report_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
            script_ref = register("campaign-selection.cmake", script_path)
            report_ref = register("campaign-selection.json", report_path)
