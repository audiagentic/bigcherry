...
// patches/1240_hi119_moe_glu_file_cli/patch.py.
...
# b11126 reformatted usage() into an aligned option table; the anchor is the
# --test-file row plus the following -j row (string contents are blanked by
# strip_noise, so two adjacent rows keep the match unique), and the new row
# is inserted between them in the same aligned style.
_USAGE_TEST_FILE_ROW = '''    printf("  --test-file <path>          read test operators from a test file generated by test-export-graph-ops\\n");'''
_USAGE_J_ROW = '''    printf("  -j <n>                      run tests using <n> parallel worker threads (default: 1, test mode only)\\n\\n");'''
_USAGE_ANCHOR_SOURCE = _USAGE_TEST_FILE_ROW + "\n" + _USAGE_J_ROW
_USAGE_NEW = (
    _USAGE_TEST_FILE_ROW + "\n"
