...
CONFIRMED via deeper repo-validated dev-gpt review (2026-09-10), consistent with this item's own real-hardware result above: run only split-mode discrimination (done -- see real hardware result above). Existing PGC01/PGC02 do NOT own MTP tensor placement, so if a tensor-split graph fix is pursued, it needs a genuinely NEW patching-owned item, not scope expansion of this one -- confirms the same conclusion already recorded above from the real -sm layer/-sm none test.
