...
TODO. GPT design request stalled/unresponsive after 2 attempts (req_6a45d0917b114112 failed EXT-GPTAUTO-003 composer-operation-timeout; retry req_3710dbae73fb49be ran >5 min with no terminal response and could not be waited out under the single-in-flight-request cap without blocking every other item in this batch) -- design below was authored directly from verified b11126 source instead. Relevance: no existing patch (grep of patches/*/patch.toml for RD47 found nothing); AMD-FUS-003 is not upstream-absorbed (b11126's op-fusion matcher ggml_cuda_can_fuse/ggml_cuda_try_fuse in ggml-cuda.cu has no MUL_MAT->VIEW->ADD pattern -- only mul_mat+bias+glu, rms_norm+mul+rope[+set_rows], rope+set_rows, and MoE weighted-reduction/topk-moe patterns, confirmed by direct read of ggml-cuda.cu:3180-3270 and 3432+ at b11126). Disposition: TODO, hardware-untested design.

## Steps

1. Add a new op-sequence pattern {GGML_OP_MUL_MAT, GGML_OP_VIEW, GGML_OP_ADD} (and the GGML_OP_RESHAPE variant, since ggml_reshape emits GGML_OP_RESHAPE not GGML_OP_VIEW when the source is contiguous) to ggml_cuda_can_fuse's is_equal() checks, following the exact structural pattern already used there for mul_mat_glu_ops. 2. Write ggml_cuda_should_fuse_gemv_view_add(mul_mat, view, add) that rejects unless: mul_mat produces ne1==1 (a GEMV, not a full matmul), the view/reshape is contiguous with identical byte layout to its source (ggml_is_contiguous(view) && view->nb[0]==mul_mat->nb[0]), the add's other operand (the residual) is not itself read anywhere else in the same fused span (no aliasing), and add's op_params encode plain addition (no scale). 3. Call ggml_cuda_check_fusion_memory_ranges (existing helper) on the 3-node span before accepting. 4. In ggml_cuda_try_fuse, on GGML_OP_MUL_MAT nodes, try the new matcher first (mirroring the existing GGML_OP_MUL_MAT / GATED_DELTA_NET / topk-moe dispatch chain there); on match, write the GEMV kernel's output pointer directly to the residual add's destination buffer and skip 2 nodes. 5. Add the BIGCHERRY_PATCH_TRACE activation marker (atomic_flag + GGML_LOG_WARN once-per-process, matching 1204_rd08/1203's precedent) at the point the fused path is actually taken. 6. Add test-backend-ops.cpp GGML_OP_MUL_MAT+VIEW+ADD graph-fusion test cases (contiguous positive case, non-contiguous/strided negative case, residual-reused-elsewhere negative case) following the existing fusion test pattern in that file (search test-backend-ops.cpp for other *_FUSE_* test classes as the template). 7. Build package patches/<order>_rd47_gemv_view_add_fuse/ per bigcherry-patch-author conventions.
...
ggml/src/ggml-cuda/ggml-cuda.cu (fusion matcher + try_fuse), ggml/src/ggml-cuda/mmvq.cu or mmv.cu (epilogue-add kernel argument threading, exact file depends on which GEMV path -- MUL_MAT_VEC_Q vs MUL_MAT_VEC_F -- the target hybrid/GDN graphs use; must be confirmed at implementation time by tracing an actual captured hybrid/GDN decode graph), tests/test-backend-ops.cpp (new fusion test cases), new package patches/<order>_rd47_gemv_view_add_fuse/{patch.toml,patch.py,README.md,SUMMARY.md}.
