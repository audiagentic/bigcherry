...
1. Hard-require PRBE29's F16 shadow (and optionally PRBE30's K-padding) to exist first.
2. NEW SCOPE (not in the original item text but required by the real source state): add a hipBLASLt-specific GEMM path alongside the existing `ggml_cuda_mul_mat_cublas_impl` cuBLAS path in ggml-cuda.cu, gated to HIP builds, since hipBLASLt is a distinct API from hipBLAS's cuBLAS-compat shim and needs its own handle/algorithm-selection/tuning-cache plumbing.
3. Define Qwen3.6-27B Q8_0 primary dense signatures, Q4/Q6 secondary.
4. Sweep M=64,128,192,256,384,512,768,1024,2048,4096 comparing three arms: native quantized MMQ, default (untuned) hipBLASLt-over-shadow, tuned hipBLASLt-over-shadow.
5. Validate tolerant/exact output and PPL for all three arms BEFORE any timing comparison.
6. Record per-architecture/per-quant kernel and E2E PP, VRAM, and load cost; derive the crossover point from measurement, not an assumed fixed threshold.
7. Include M<=128, decode, and memory-constrained controls.
8. If tuned native MMQ always wins (plausible given informal general priors around GEMM crossover points, but must be measured not assumed), explicitly reject shadow dispatch as a promoted path while retaining EC10-style negative evidence -- do not silently drop a negative result.
