...
1. Create patches/<next-order>_rd05_wmma_fa_tileq_sync/ (RD05-only) copying rd05-k00-sync and rd05-kbc-sync FilePatch/Edit definitions verbatim from patches/1203_rd050607_rdna4_wmma_fa_q6k_mmq/patch.py's fattn-mma-f16.cuh FilePatch (drop rd0506-config-table and rd06-dkq-gate). 2. In the same package, add a second FilePatch for fattn.cu containing only the <atomic> include edit and a NEW RD05-only activation marker (unconditional GGML_LOG_WARN at the BEST_FATTN_KERNEL_MMA_F16 dispatch case; do not include the RD06-gated nested block from 1203's rd0506-activation-markers). 3. Create patches/<next-order>_rd07_q6k_mmq_scale_fold/ copying rd07-hoist-base-scale, rd07-fold-subscale, rd07-sum-line (mmq-vec-dot.cuh, in that order) verbatim from 1203. 4. In the RD07 package add mmq.cu's rd07-atomic-include + rd07-activation-marker (Q6_K dispatch marker), and mmq.cuh's rd07-jmax-env (J_MAX override), and tests/test-backend-ops.cpp's rd07-perf-cases -- all copied verbatim from 1203. 5. Write patch.toml for each (state=untested, kind=enhancement, experiment-contracts=["RD05-WMMA-FA-CORRECTNESS-BARRIERS"] scoped to the tileQ-sync claim only, and ["RD07-Q6K-MMQ-PREFILL-FOLD"] respectively; requires=[] for both -- they are anchor-independent of each other and of 1203/1204). 6. Write SUMMARY.md for each documenting the split provenance (this plan item, PA39 rejection, req_f34f50a25c6240fe). 7. Run patch-lint and patch-rebase-check for both packages standalone and together with the existing production patchset. 8. Do NOT reintroduce rd06-*, rd0506-config-table, or rd0506-softcap-read in either package.
