...
1. This item's substantive qualification work (activation, bit_identical, 10-round CI95 performance evidence) is done per notes. Remaining formal step: wiring run_rd39_42_contract_qualification() (GPT-approved design, req_3e42043eb71a4a92) into validation_campaign.py -- the central run_rd39_42_contract_qualification()/run_rd08_contract_qualification()-style pattern GPT expected does not exist as a generic runner; author it as a dedicated function in tools/bigcherry/patch/validation_campaign.py following the RD08/RD73 pattern.
2. Add `experiment-contract = "RD39-42-STREAM-MOE-OVERLAP"` to patches/1215_rd394041_amd_stream_moe_overlap/patch.toml (verified: patch.toml currently has no experiment-contract binding field) so patch-verify-evidence can resolve the contract.
3. Add patches/1215_rd394041_amd_stream_moe_overlap/validation.toml plus a patch-local validation/producer.py using the validation_producer.py protocol (see patches/1216_rd43_concurrent_join_fusion_guard/validation/ for the sibling pattern), producing: bit_identical evidence (raw logit digest, already gathered per notes), activation evidence (marker-fire count), paired-performance evidence (10-round CI95, already gathered), and control-regression evidence (10-round CI95, already gathered).
4. Before rebase-check/promotion, confirm which of 1215's Edit()s duplicate now-native upstream per-stream cublas_handle/pool machinery (PRBE32, confirmed upstream-absorbed) vs which are the still-necessary MoE-fan-out-specific dedicated-scratch piece (PRBE33, confirmed NOT upstream-absorbed, retained) -- do not trim rd3942-scratch-fields/destructor/concurrent_groups on the mistaken premise both prerequisites are absorbed.
...
Real anchored context (b11126 ggml/src/ggml-cuda/ggml-cuda.cu, function ggml_backend_cuda_graph_optimize): the existing generic detector computes fan_out[src] per source tensor, requires count in [min_fan_out,max_fan_out] (both hardcoded to 3), and only proceeds if strstr(root_node->name, "attn_norm"). A MoE shared-expert diamond has fan_out==2, root is an FFN/MoE input norm not attn_norm. A future slimmer patch could extend this detector to the 2-way MoE case, reusing native stream_mapping/concurrent_event/join_events/cublas_handle()/pool() -- flagged as follow-on design, not required for this item's disposition. This item's disposition (qualify 1215 as-is) requires only the contract-binding + validation.toml + producer wiring above; 1215's own scratch/stream plumbing must NOT be trimmed on PRBE33's now-corrected finding that dedicated scratch is still needed (not upstream-duplicated).
...
patches/1215_rd394041_amd_stream_moe_overlap/patch.py (rebase-check target, no source edit here); patches/1215_rd394041_amd_stream_moe_overlap/patch.toml (add experiment-contract binding); patches/1215_rd394041_amd_stream_moe_overlap/validation.toml (new); patches/1215_rd394041_amd_stream_moe_overlap/validation/producer.py (new, patch-local, validation_producer.py protocol); tools/bigcherry/patch/validation_campaign.py (add run_rd39_42_contract_qualification()); config/experiment-contracts.toml ([contract.RD39-42-STREAM-MOE-OVERLAP], already exists per notes, needs qualification runner wiring only).
...
Offline: PYTHONPATH=tools python -m bigcherry patch-lint patches/1215_rd394041_amd_stream_moe_overlap; patch-rebase-check --focal-overlay 1215_rd394041_amd_stream_moe_overlap --source bigcherry-tuning; unit test for run_rd39_42_contract_qualification() against the RD08/RD73 test pattern. Hardware (Brutus, gfx1100/gfx1201, not gfx1151 -- Brutus has no gfx1151 host; this item's target architectures per its own contract are gfx1100/gfx1201/gfx1030, so Brutus is a correct qualification host here): run run_rd39_42_contract_qualification() once authored on tierM-qwen35b-a3b-moe-mtp to produce the formal eligible_for_validated_state verdict; underlying evidence already gathered per notes does not need regathering.
