...
TODO, source identity corrected -- this is two alternative candidates, not one compatible layout. GPT identified: AMD PR #84 (head 1b600a34...) is an RDNA3.5/gfx1151 x4-activation + wide-load + unroll/barrier + nwarps experiment; PR #88 (merged as 446d0cee...) is a DIFFERENT RDNA3.5 VDR=8/b128 path whose activation explicitly remains ordinary SCALAR block_q8_1 (not x4-wide). These are alternative, mutually-distinct candidates -- NOT one 'wide-load producer feeds VDR=8 consumer' combined layout as the prior draft's step 3 legality-assertion language implied. Both are gfx1151-scoped (RDNA3.5), not gfx1201-primary/gfx1100-holdout as the prior draft assumed. PR #84 additionally requires `GGML_CUDA_DQ_MMV=0` to even reach the MMVQ dispatch path on gfx1151 (its own stated activation prerequisite) -- the prior validation plan omitted this and would silently fail to activate the candidate it's supposed to test.
...
1. Freeze both source identities exactly: PR #84 head 1b600a34... (x4-activation/wide-load/unroll/barrier/nwarps, gfx1151) and PR #88 merged 446d0cee... (VDR=8/b128, scalar block_q8_1 activation, gfx1151) -- treat as two SEPARATE candidate patches, each independently gated, never combined into one 'wide producer + VDR=8 consumer' hybrid unless a THIRD, explicitly-designed hybrid is later proposed with its own correctness case.
2. Read the full body of `vec_dot_q4_K_q8_1` (vecdotq.cuh ~918) and its mmvq.cu dispatch sites as originally scoped (step 1 of the prior draft), but evaluate each of PR #84 and PR #88's real diffs against that code separately -- do not conflate their activation/layout assumptions.
3. Each candidate gets its own gated env var and its own separately-compiled/templated code path with a host selector (e.g. GGML_CUDA_MMVQ_Q4K_VARIANT=native|pr84_x4|pr88_vdr8), never a runtime mutation of a compile-time constexpr VDR/layout assumption (the prior draft's per-request VDR toggle idea is unsafe for this reason).
4. Primary qualification architecture is gfx1151 (RDNA3.5, both PRs' actual target), with `GGML_CUDA_DQ_MMV=0` set as an explicit prerequisite env var alongside each candidate's own selector when testing PR #84 specifically (confirm whether PR #88 has the same or a different prerequisite by reading its actual diff before assuming).
5. gfx1100/gfx1201 are NON-SELECTION CONTROLS ONLY (confirm the env-gated candidates do not activate and produce byte-identical native output there) -- they are not the primary qualification target as the prior draft assumed; broadening to those architectures as a real qualification target is a separate, later decision requiring its own evidence, not assumed here.
6. Correctness: repeated fixed-seed direct MUL_MAT_VEC test-backend-ops cases for Q4_K at long K, each candidate independently, on gfx1151 (primary, with its correct prerequisite env var) and gfx1100/gfx1201 (non-selection controls), plus a Q8_0 unaffected-quant control.
7. Only after correctness passes per candidate: kernel time and end-to-end decode t/s on gfx1151, reported per candidate separately, never combined.
...
2026-09-24 GPT review req_d55aed71224e43a8 applied: NOT-READY -- froze PR #84 (1b600a34, x4-activation/wide-load, gfx1151) and PR #88 (446d0cee, VDR=8/b128, scalar activation, gfx1151) as separate alternative candidates, not one compatible hybrid; corrected primary qualification architecture from gfx1201 to gfx1151; added PR #84's GGML_CUDA_DQ_MMV=0 activation prerequisite.
...
- 2026-09-24T04:47:47.971743+00:00 (updated-by): Updated: section:description, section:steps
- 2026-09-24T04:47:50.879941+00:00 (updated-by): Updated: section:notes
