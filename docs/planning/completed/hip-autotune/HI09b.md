...
- ~~Populate the existing `candidate_blacklist` SQLite table for tune builds~~
  — STRUCK 2026-08-20 (PH01 audit): the table has zero writers and zero readers
  in the tree; catalog-level exclusion via the report JSON is the source of
  truth (standard 2.5). A second SQLite copy would duplicate, not add.
...
### 2026-08-20 PH01 audit — complete; one step struck as redundant

Verified ~90% implemented: `tools/bigcherry/resource_report.py` (strict remark parser, `ResourcePolicyV1` with reject_scratch_gt/lds/occupancy, mangled→stable-name `_resolve` mapping), catalog-level exclusion via `apply_resource_blacklist` + `--resource-report` CLI (`autotune_catalog.py:274/:745/:1551`), tests in `tools/tests/test_resource_report.py`, ledger chg_20260813_071005. The item's own "important choice" — exclude at catalog level so the TU never compiles — is the shipped mechanism.

The single unimplemented step (populate the `candidate_blacklist` SQLite table) is struck: repo-wide grep shows the table is defined at `sql/dispatch-db.sql:319` with zero writers and zero readers (the `dispatch.cu:592` runtime "blacklist" is a separate winner-entry mechanism). Standards 2.5 makes the report JSON the one source of truth. Closure: completed.
...
- 2026-08-20T07:24:02.594817+00:00 (updated-by): Updated: section:steps, section:notes
- 2026-08-20T07:24:14.258676+00:00 (state-transition): State: in_progress → completed
- 2026-08-20T07:38:27.738178+00:00 (updated-by): Updated: section:ledger-events
