...
| A5 | **Execute the rebase**: `pull` recipes to b10502 → audit (33/33 verified) → apply production selection (verified clean) → **full ROCm builds** (local + Brutus) → smoke (server incl. 0800/1005 endpoints, llama-bench dense + MoE, iGPU load path, MTP crash repro per HI71/EX02). | The actual rebase; compile gate is the real risk | L | Partial — a `control` build (single arch, gfx1201) with the ported framework patch compiled successfully. Full production-selection build across all archs + smoke suite still open. |
