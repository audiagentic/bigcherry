...
4. Session B unstages/removes only B's probe, verifies A's probe is still present and byte-identical, and signals completion. Session A then verifies its probe is byte-identical, removes only A's probe, and performs the final clean-checkout assertion. (B must not require a clean checkout while A's deliberately-held probe still exists.)
