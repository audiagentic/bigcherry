...
- **Profiling/diagnostics:** hardware-facing diagnostic harnesses that are genuinely isolated from production selection -- e.g. `rocprof.py` (kernel-trace capture/parsing) and the RCCL heterogeneous-topology qualification tools (`rccl_qualify.py`, `rccl_qualify_campaign.py`); owner `bigcherry.profiling`. Crash-isolated (one case per subprocess) by design where the collective under test can hard-abort. See `docs/reference/testing/RCCL_HETEROGENEOUS_RUNBOOK.md` for the governing procedure and current usage of the RCCL qualification tools specifically -- do not duplicate that tooling into a new `bigcherry/rccl/` package or an ad-hoc one-off script; extend the existing driver instead (tracked as GP07 in `docs/planning/active/gpu-collectives/`). A VALIDATE (crash-safety/correctness qualification) result from this domain is a hard prerequisite for any OPTIMIZE (performance comparison) run in `bigcherry.campaign` that depends on RCCL -- an optimization arm must never run, let alone silently downgrade, against an unqualified topology/RCCL-revision combination (GP08).
