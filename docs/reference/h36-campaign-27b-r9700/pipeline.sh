...
exec 9>"$BIN/.lock"
flock -n 9 || { echo "another pipeline instance holds $BIN/.lock; exiting" >&2; exit 1; }
MASTER=$LOG/master.log

say() { echo "[$(date '+%F %T')] $*" | tee -a "$MASTER"; }
fail() { say "FAILED: $*"; echo "FAILED: $*" > "$STATUS/current"; exit 1; }
...
say "S6 start: export replay cache from tune run 1"
...
( cd "$BC/tools" && python3 -m bigcherry.replay_cache "$BIN/tune-t1.jsonl.measurements.jsonl" \
...
replay_run() {
  local tag=$1 port=$2 cfg=$3 toggles=$4 extra=$5
