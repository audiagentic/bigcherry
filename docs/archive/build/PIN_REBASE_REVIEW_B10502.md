...
| `25ae3a9b` #26843 | MMVQ `small_k`/`halve_iters` params + GB10 table (nwarps=8 bs=1 dense, DGX Spark) | 0600/0650 re-anchored (done). Upstream "shape-aware warp count" convergence continues — third signal (#20831, #27233, #26843) that our nwarps-table work is converging with upstream; relevant to RD21's long-term dedup question. **S1b-confirmed (2026-08-21)**: the added `halve_iters` template param changes the mangled kernel name of the non-fusion small_k mmvq instantiation between pins (6→7 template args). Behaviour verified identical (identical dispatch count, busy to 0.02pp, identical geometries, ceiling Δ0.018pp — docs/evidence/2026-08-21-hi35-hi36-27b-r9700/README.md S1b section), but **kernel-name-keyed caches are invalidated by this pin move by construction**; v4 `rerun_required` on cross-pin caches is the correct production path (input to HI74). |
