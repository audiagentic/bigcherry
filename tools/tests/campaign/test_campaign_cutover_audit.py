...
from bigcherry.build.builds import build_directory  # noqa: E402
from bigcherry.campaign.lane import (CampaignLaneError, CampaignLaneExecutionSpec,  # noqa: E402
...
def _fake_compiler(calls: list):
    ...
    (whole-module import) means patching ``bigcherry.campaign.workers.
...
class _Harness:
    ...
    def run(self, *, run_id: str, allow_dirty_bigcherry: bool = True, calls: list | None = None):
        ...
        with patch("bigcherry.campaign.workers.subprocess.run", _fake_compiler(calls)):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            def run_with_arch(architectures, run_id):
                ...
                def fake_make_generate_worker(*, context, source_root, run_id,
                    ...
                    from bigcherry.build import generated_tree as gt
                ...
                with patch("bigcherry.campaign.workers.subprocess.run", _fake_compiler(calls)), \
                     patch("bigcherry.campaign.workers.make_generate_worker",
...
class RuntimeBundleIntegrityTests(unittest.TestCase):
    ...
    def test_tampering_a_published_so_member_fails_smoke_closed(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            harness = _Harness(root, with_patch=False)
            from bigcherry.campaign.smoke import RuntimeSmokeSpec
            ...
            with patch("bigcherry.campaign.workers.subprocess.run", side_effect=fake_subprocess):
                result = execute_campaign_lane(
                    spec, cfg=harness.cfg, context=harness.context, store=harness.store,
