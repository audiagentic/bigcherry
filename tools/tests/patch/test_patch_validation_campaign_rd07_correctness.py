"""RD07 backend_reference orchestration tests -- hardware-free."""
...
class RunRd07ContractCorrectnessTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _run(self, *, architecture: str, subject_ppl: float, control_ppl: float, uncertainty: float = 0.0275):
        ...
        result = vc.run_rd07_contract_correctness(
            base_revision="a" * 40, hip_path=Path("H:/hip"), amdgpu_targets=architecture,
            worktree_root=Path("W:/worktrees"), build_root=Path("B:/build"),
            model=Path("M:/tierM-gptoss20b-q6k.gguf"), corpus=Path("C:/wikitext2/test.txt"),
        ...
        for architecture in ("gfx1100", "gfx1201", "gfx1030"):
            with self.subTest(architecture=architecture):
                result, run_dir, source_module = self._run(
                    architecture=architecture, subject_ppl=10.3938, control_ppl=10.4463,
                )
                ...
                self.assertEqual(doc["contract_id"], "RD07-Q6K-MMQ-PREFILL-FOLD")
    ...
    def test_large_ppl_divergence_is_a_correctness_failure(self) -> None:
        result, _, _ = self._run(
            architecture="gfx1100", subject_ppl=15.0, control_ppl=10.0, uncertainty=0.001,
        )
    ...
    def test_wrong_architecture_fails_closed_before_materialization(self) -> None:
        ...
        with self.assertRaises(vc.PatchCampaignError):
            vc.run_rd07_contract_correctness(
                base_revision="a" * 40, hip_path=Path("H:/hip"), amdgpu_targets="gfx1151",
                worktree_root=Path("W:/worktrees"), build_root=Path("B:/build"),
                model=Path("M:/tierM-gptoss20b-q6k.gguf"), corpus=Path("C:/wikitext2/test.txt"),
    ...
    def test_multi_architecture_build_fails_closed_before_materialization(self) -> None:
        ...
        with self.assertRaises(vc.PatchCampaignError):
            vc.run_rd07_contract_correctness(
                base_revision="a" * 40, hip_path=Path("H:/hip"), amdgpu_targets="gfx1100;gfx1201",
                worktree_root=Path("W:/worktrees"), build_root=Path("B:/build"),
                model=Path("M:/tierM-gptoss20b-q6k.gguf"), corpus=Path("C:/wikitext2/test.txt"),
                run_dir=Path(tempfile.mkdtemp()), _source_module=source_module,
            )
