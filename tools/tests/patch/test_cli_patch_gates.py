...
def _canonical_selection(
    ...
    *,
    ...
    """A real CanonicalSelection (PA34: identity is the sole authority) for
    mocking resolve_canonical_selection -- the handler reads
    ``selection.identity``, so the mock must carry the real shape."""
    from bigcherry.campaign.resolution import CanonicalSelection, SelectorIdentity
    ...
    identity = SelectorIdentity(
        ...
        source_ref="b10901",
        patch_set_id="ps-test",
        patch_ids=tuple(patch_ids),
        module_hashes=tuple((pid, "deadbeef" * 8) for pid in patch_ids),
    )
    return CanonicalSelection(identity=identity)
...
class PatchGatesCliTests(unittest.TestCase):
    ...
    def test_json_reports_all_applicable_gates_and_pass_exit(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                cli_patch.patch_registry, "load_registry", return_value=self.registry
            ),
            mock.patch.object(
                cli_patch.patchset, "catalog", return_value=[self.module]
            ),
            mock.patch.object(
                cli_patch.patchset,
                "expand_composition",
            ...
            mock.patch.object(
                cli_patch.patchset, "resolve_exact", return_value=self.composition
            ),
            mock.patch.object(cli_patch, "UpstreamRepository", return_value=self.repo),
            mock.patch.object(
                cli_patch.patch_gates, "evaluate_patch_gates", return_value=results
            ),
        ):
            output = StringIO()
        ...
        self.assertEqual(
            [gate["id"] for gate in payload["gates"]], ["G0", "G1", "G2", "G3"]
        )
    ...
    def test_fail_or_blocked_gate_returns_one(self) -> None:
        ...
        results = (
            GateResult(
                GateId.G0, GateStatus.BLOCKED, "composition", "test", ("missing",)
            ),
        )
        with (
            mock.patch.object(cli_patch.campaign_config, "load", return_value=self.cfg),
    ...
    def test_source_build_forwards_ordered_composition_and_gate_context(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                cli_patch.patchset, "catalog", return_value=[first, focal]
            ),
            mock.patch.object(
                campaign_resolution,
                "resolve_canonical_selection",
                return_value=_canonical_selection(("A", "P1")),
            ) as resolve_source,
            mock.patch.object(
                cli_patch.patchset,
                "resolve_exact",
            ...
            mock.patch.object(
                ...
                "load_report",
                side_effect=(rebase_report, all_report),
            ) as load_report,
            mock.patch.object(cli_patch, "UpstreamRepository", return_value=self.repo),
            mock.patch.object(
                cli_patch.patch_gates, "evaluate_patch_gates", return_value=results
            ...
            output = StringIO()
            with redirect_stdout(output):
                exit_code = cmd_patch_gates(
                    _args(
                        intent="build",
                        source="bigcherry",
                        rebase_report="rebase.json",
                        all_report="all.json",
        ...
        resolve_source.assert_called_once_with(
            ...
            experiment=None,
            focal_overlay_patch_id=None,
        )
        resolve_exact.assert_called_once_with(
            ("A", "P1"),
            directory=cli_patch.paths.PATCHES,
        ...
        self.assertEqual(
            tuple(m.patch_id for m in context.composition.modules), ("A", "P1")
        )
        ...
        self.assertEqual(
            self.repo.resolve_ref.call_args_list[0].args, (self.cfg.pinned,)
        )
        self.assertEqual(self.repo.resolve_ref.call_args_list[1].args, ("HEAD",))
        self.assertIn('"composition": [', output.getvalue())

    def test_source_mode_reports_not_evaluated_when_focal_outside_selection(
        self,
    ) -> None:
        # PA34: a focal patch not in the resolved selector is an explicit
        # NOT_EVALUATED selection-status result (exit 1, no gates), not a
        # resolution error.
        import bigcherry.cli.patch as cli_patch
        ...
        with (
            ...
            mock.patch.object(
                campaign_resolution,
                "resolve_canonical_selection",
                return_value=_canonical_selection(("OTHER",)),
            ) as resolve_source,
        ...
        payload = json.loads(output.getvalue())
        self.assertEqual(payload["selection_status"], "NOT_EVALUATED")
    ...
    def test_focal_promote_expands_dependency_closure_without_recipe_ids(self) -> None:
        ...
        with (
            ...
            mock.patch.object(
                cli_patch.patchset, "catalog", return_value=[dependency, focal]
            ),
            mock.patch.object(
                cli_patch.patchset,
                "expand_composition",
    ...
    def test_rebase_source_mode_is_successfully_wired(self) -> None:
        ...
        with (
            ...
            mock.patch(
                ...
                return_value=_canonical_selection(("P1",)),
            ),
            mock.patch.object(
                cli_patch.patchset, "resolve_exact", return_value=composition
            ),
            mock.patch.object(
                cli_patch.patch_rebase, "load_report", return_value={"report": "r"}
            ),
            mock.patch.object(cli_patch, "UpstreamRepository", return_value=self.repo),
            mock.patch.object(
                cli_patch.patch_gates, "evaluate_patch_gates", return_value=results
    ...
    def test_human_output_is_derived_from_json_result_shape(self) -> None:
        ...
        self.repo.resolve_ref.side_effect = lambda ref: (
            "a" * 40 if ref == self.cfg.pinned else "b" * 40
        )
        with (
            mock.patch.object(cli_patch.campaign_config, "load", return_value=self.cfg),
            mock.patch.object(
    ...
    def test_parser_registers_patch_gates_and_required_intent(self) -> None:
        ...
        args = parser.parse_args(
            ["patch-gates", "P1", "--intent", "validate", "--json"]
        )
