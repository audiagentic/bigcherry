...
from bigcherry.campaign import comparison as comparisons
from bigcherry.core import provenance # noqa: E402
from bigcherry.core.artifacts import ArtifactStore  # noqa: E402
from bigcherry.tuning.promotion import PromotionError, pointer_from_comparison_report  # noqa: E402
...
class RunComparisonTests(unittest.TestCase):
    def test_balanced_comparison_produces_a_valid_decision_grade_report(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("bigcherry.campaign.benchmark.subprocess.run", _fake_run()):
    ...
    def test_entrypoint_and_binary_hash_mismatch_rejects_before_execution(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "bigcherry.campaign.benchmark.subprocess.run", _fake_run()
            ) as fake_run:
                with self.assertRaises(comparisons.ComparisonError):
    ...
    def test_non_exact_replay_coverage_makes_the_report_invalid_not_an_exception(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "bigcherry.campaign.benchmark.subprocess.run", _fake_run(replay_exact=False)
            ):
                report_ref = comparisons.run_comparison(
                    plan,
                    store=fx.store,
