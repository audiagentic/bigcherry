...
RECORD_OBS = {
    ...
    "canonical": {
        "op": "MUL_MAT", "src0_type": 8, "src1_type": 0, "dst_type": 0,
        "ne0": [64, 512], "ned": [64, 128],
    },
    "hardware_key": {
        "architecture_code": "gfx1100", "wave_size": 64,
        "compute_units": 60, "feature_flags": 1,
    },
    "calls": 5, "est_bytes": 2048, "devices": [0],
}
...
TUNING_RESULT_NATIVE = {
    ...
    "improvement_pct": 0.0,
    "generated": 3, "eligible": 3, "measured": 2,
    "reason": "native retained",
    "candidates": [{
        "name": "mmq:native:v1", "status": "ok",
        "median_us": 1.5, "mad_us": 0.01, "p95_us": 1.6, "host_median_us": 0.4,
    }],
}
...
def _fake_run(*, record_jsonl: str | None = None, tune_jsonl: str | None = None,
              coverage: bool = False):
    ...
    def run(argv, *, capture_output, text, env):
        if coverage:
            Path(env["GGML_HIP_DISPATCH_COVERAGE"]).write_text(json.dumps({
                "total_dispatched": 1, "total_executed": 1,
                "replay": {
                    "schema_version": 2, "exact": 1, "candidate_unavailable": 0,
        ...
        if tune_jsonl is not None:
            Path(db_path + ".measurements.jsonl").write_text(tune_jsonl, encoding="utf-8")
        return _FakeCompletedProcess()
...
class _Fixture:
    ...
    def __init__(self, directory: Path):
        ...
        self.run_id = "run1"
        self.seed_doc = provenance.ProvenanceV2.from_document(provenance.make(
...
class RecordStageTests(unittest.TestCase):
    def test_record_stage_publishes_a_valid_record_jsonl(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            rehydrated = fx.store.rehydrate(result.record_ref.artifact_id, expected_kind="record-jsonl")
    ...
    def test_record_stage_fails_closed_on_a_tampered_bundle_member(self):
        with tempfile.TemporaryDirectory() as directory:
            fx = _Fixture(Path(directory))
            (fx.store.root / "builds/s1/bp1/entrypoint.py").write_text("TAMPERED", encoding="utf-8")
            with patch("bigcherry.lifecycle.subprocess.run", _fake_run(record_jsonl=_RECORD_JSONL)) as fake_run:
    ...
    def test_record_stage_rejects_a_bare_ref_with_no_artifact_id(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaises(lifecycle.LifecycleError):
                lifecycle.execute_record_stage(
                    context=fx.context, store=fx.store, run_id=fx.run_id,
                    runtime_bundle=bare, spec=fx.spec,
...
class InventoryStageTests(unittest.TestCase):
    def test_inventory_stage_publishes_inventory_and_database(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            rehydrated_db = fx.store.rehydrate(result.database_ref.artifact_id, expected_kind="dispatch-db")
            self.assertEqual(rehydrated_db.content_hash, result.database_ref.content_hash)

    def test_inventory_stage_derives_campaign_identity_from_a_complete_parent_triple(self):
    ...
    def test_inventory_stage_falls_back_to_legacy_imported_without_a_complete_parent_triple(self):
        with tempfile.TemporaryDirectory() as directory:
            fx = _Fixture(Path(directory))
            fx.seed_doc = provenance.ProvenanceV2.from_document(provenance.make(
                project={"provenance_class": "development", "bigcherry_revision": "r1"},
                source={"source_slice_id": "s1"}, build={}, workload={}, campaign={"run_id": "seed"},
            ))
            ...
            fx.bundle_ref = fx.store.publish_json_ref(
                "builds/s1/bp1/runtime-bundle-2.json", manifest,
...
class TuneStageTests(unittest.TestCase):
    def _inventory(self, fx: _Fixture):
        ...
        return lifecycle.execute_inventory_stage(
            context=fx.context, store=fx.store, run_id=fx.run_id,
            record=ArtifactLocator(record_ref.artifact_id), local_provenance_class="development",
        )
    ...
    def test_tune_stage_publishes_measurements_and_updated_database(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("bigcherry.lifecycle.subprocess.run", _fake_run(tune_jsonl=_TUNE_JSONL)):
            ...
            self.assertTrue(fx.store.verify(
    ...
    def test_tune_stage_fails_before_running_when_the_db_artifact_was_tampered(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("bigcherry.lifecycle.subprocess.run", _fake_run(tune_jsonl=_TUNE_JSONL)) as fake_run:
                with self.assertRaises(lifecycle.LifecycleError):
    ...
    def test_promotion_stage_publishes_promoted_winners(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            measurements_bytes = _jsonl(
                {"kind": "header", "artifact_version": 1, "source_revision": "b" * 40,
                 "manifest_hash": "a" * 32},
                {
                    "kind": "result", "dispatch": "a" * 32, "native": "native",
                    "signature": "a" * 32, "winner": "candidate",
                    "promotion_status": "pending_bh", "provisional_winner": "candidate",
                    "schedule_seed": int.from_bytes(bytes.fromhex("a" * 32)[:4], "little"),
                    "schedule": {"schema_version": 1, "selection_algorithm": "seeded-rotation-v1",
                                 "confirmation_algorithm": "seeded-alternation-v1",
                                 "candidates": ["candidate", "native", "native#twin"]},
                    "improvement_pct": 5.0,
                    "confirmation": {"p_value": 0.001, "effect_pct": 5.0, "wins": 12, "rounds": 12,
                                      "native_us": [100.0] * 12, "winner_us": [95.0] * 12},
                },
            ).encode("utf-8")
            measurements_ref = fx.store.publish_bytes_ref(
                "runs/run1/tune/measurements.jsonl", measurements_bytes,
                kind="tuning-measurements", provenance=fx.seed_doc)
            result = lifecycle.execute_promotion_stage(
                context=fx.context, store=fx.store, run_id=fx.run_id,
                measurements=ArtifactLocator(measurements_ref.artifact_id),
                resamples=1000, local_provenance_class="development",
            )
            self.assertEqual(result.promoted_winners_ref.kind, "promoted-winners")
            rows = [json.loads(line) for line in result.promoted_winners_ref.path.read_text().splitlines()]
    ...
    def test_replay_export_stage_publishes_a_replay_cache(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            winners_bytes = _jsonl(
                {"kind": "header", "source_revision": "b" * 40, "manifest_hash": "a" * 32},
                {"kind": "result", "dispatch": "A" * 32, "signature": "C" * 32,
                 "winner": "mmvq:native:v1", "native": "mmvq:native:v1"},
            ).encode("utf-8")
            winners_ref = fx.store.publish_bytes_ref(
                "runs/run1/promote/promoted-winners.jsonl", winners_bytes,
                kind="promoted-winners", provenance=fx.seed_doc)
            manifest_doc = {
                "source_revision": "b" * 40, "manifest_hash": "a" * 32,
                "candidates": [{
            ...
            manifest_ref = fx.store.publish_json_ref(
                "runs/run1/manifest.json", manifest_doc, kind="manifest", provenance=fx.seed_doc)
            ...
            result = lifecycle.execute_replay_export_stage(
                ...
                source_root=source_root, local_provenance_class="development",
            )
    ...
    def test_replay_validation_stage_publishes_full_coverage(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            cache_ref = fx.store.publish_bytes_ref(
                "runs/run1/replay/replay.cache", b"fake-cache-bytes",
                kind="replay-cache", provenance=fx.seed_doc)
...
class RehydrationAcrossFreshStoreTests(unittest.TestCase):
    def test_every_stage_output_rehydrates_in_a_fresh_artifact_store_instance(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            rehydrated = fresh_store.rehydrate(record_ref.artifact_id, expected_kind="record-jsonl")
