...
from bigcherry import replay_inspect  # noqa: E402
from bigcherry.autotune_catalog import build_descriptor  # noqa: E402
...
class ReplayInspectToolTests(unittest.TestCase):
    ...
    def _run(self, report: dict, exit_code: int = 0):
        ...
        return replay_inspect.run_tool(
            self.fake, cache=self.cache, interpreter=[sys.executable]
        )
    ...
    def test_unparseable_output_raises(self):
        ...
        with self.assertRaises(SystemExit):
            replay_inspect.run_tool(
                broken, cache=self.cache, interpreter=[sys.executable]
            )
