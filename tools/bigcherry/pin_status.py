...
def campaign_preflight(clone_source: Path, repo_paths: RepoPaths) -> tuple[list[str], bool]:
    ...
    if status.verdict == VERDICT_MID_REBASE:
        return [], True
    return strict_failure(PinStatusReport(
        local=status, remotes=[], converged=None)), False
