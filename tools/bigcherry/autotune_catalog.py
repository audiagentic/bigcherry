...
_BLAS_PLAN_ENUM = {
    "operand_type": {
        "native": "GGML_HIP_BLAS_OPERAND_NATIVE",
        "f32": "GGML_HIP_BLAS_OPERAND_F32",
        "f16": "GGML_HIP_BLAS_OPERAND_F16",
        "bf16": "GGML_HIP_BLAS_OPERAND_BF16",
    },
    "accumulation_type": {
        "native": "GGML_HIP_BLAS_ACCUMULATION_NATIVE",
        "f16": "GGML_HIP_BLAS_ACCUMULATION_F16",
        "f32": "GGML_HIP_BLAS_ACCUMULATION_F32",
    },
    "output_type": {
        "native": "GGML_HIP_BLAS_OUTPUT_NATIVE",
        "f16": "GGML_HIP_BLAS_OUTPUT_F16",
        "bf16": "GGML_HIP_BLAS_OUTPUT_BF16",
        "f32": "GGML_HIP_BLAS_OUTPUT_F32",
    },
    "source_a_conversion": {
        "none": "GGML_HIP_BLAS_CONVERSION_NONE",
        "contiguous": "GGML_HIP_BLAS_CONVERSION_CONTIGUOUS",
        "non_contiguous": "GGML_HIP_BLAS_CONVERSION_NON_CONTIGUOUS",
    },
    "source_b_conversion": {
        "none": "GGML_HIP_BLAS_CONVERSION_NONE",
        "contiguous": "GGML_HIP_BLAS_CONVERSION_CONTIGUOUS",
    ...
    "output_conversion": {
        "none": "GGML_HIP_BLAS_OUTPUT_CONVERSION_NONE",
        "temporary_to_f32": "GGML_HIP_BLAS_OUTPUT_CONVERSION_TEMPORARY_TO_F32",
    },
    "numerical_class": {
        "exact_baseline": "GGML_HIP_BLAS_NUMERICAL_EXACT_BASELINE",
        "equivalent_within_backend_tolerance": "GGML_HIP_BLAS_NUMERICAL_EQUIVALENT",
        "reduced_precision": "GGML_HIP_BLAS_NUMERICAL_REDUCED",
    },
}
...
def _variant_initialiser(candidate: dict[str, Any]) -> str:
    ...
    #
    # BLAS is the exception and stays 0: it has no geometry variant. Its
    # structured seven-field plan is emitted through the separate side table.
...
def _blas_plan_initialiser(candidate: dict[str, Any]) -> str | None:
    """Return a compact generated plan object, or ``None`` for no plan.

    The side table is deliberately keyed by runtime_id rather than embedded in
    ``ggml_hip_variant_params``. Replay v3 can therefore continue to deserialize
    its fixed 54-byte entry while the runtime still receives the full resolved
    seven-field BLAS identity.
    """
    plan = candidate.get("config", {}).get("blas_plan")
...
def render_registry(manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "// BLAS plans are runtime metadata keyed by the stable registry id.",
    ]

    for index, candidate in enumerate(manifest["candidates"]):
        plan = _blas_plan_initialiser(candidate)
        if plan is not None:
            lines.append(
                f"static const ggml_hip_blas_plan_v1 ggml_hip_blas_plan_{index} = {plan};")

    lines += [
        "",
        "static const ggml_hip_blas_plan_v1 * const "
        "ggml_hip_blas_plan_registry[] = {",
    ]

    for index, candidate in enumerate(manifest["candidates"]):
        if _blas_plan_initialiser(candidate) is None:
            lines.append("    nullptr,")
        else:
            lines.append(f"    &ggml_hip_blas_plan_{index},")
