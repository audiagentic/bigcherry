...
from .journal import atomic_write, canonical
from ..identity_separation import IdentitySeparationError, validate_measurement_identity
from . import promotion_gate as promotion_correctness_gate
from . import ranking as ranking_policy
...
def _validated_effect(confirmation: dict[str, Any]) -> float:
    ...
    # Timing arrays are serialized to six decimal microseconds (RE28) while
    # the producer computes effect_pct from the higher-precision device
    # values. Permit the resulting sub-centipercent rounding drift, but not
    # a material effect mismatch.
